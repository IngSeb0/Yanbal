...
function catalogWhatsAppLink(page: CatalogPage) {
  const price = page.prices.length
    ? page.prices.slice(0, 3).join(", ")
    : "consultar precio";
  const code = page.codes.length
    ? ` Códigos visibles: ${page.codes.join(", ")}.`
    : "";
...
function PriceMeta({ page }: { page: CatalogPage }) {
  return (
    <div className="price-meta" aria-label="Precios y códigos visibles">
      ...
      {!page.prices.length && !page.codes.length ? (
        <span>Consultar</span>
      ) : null}
    </div>
  );
...
export default function CatalogoPage() {
  return (
    <main>
      <section
        className="hero catalog-page-hero"
      ...
      <section
        className="catalog-archive catalog-archive--standalone"
      ...
      >
        ...
        {catalogGroups.map((group) => (
          <section
            ...
            id={`catalogo-${slugify(group.section)}`}
            key={group.section}
          >
