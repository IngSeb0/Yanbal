...
export async function supabaseRequest(path, options = {}, authOptions = {}) {
  ...
  // Opaque sb_* API keys are not JWTs. Supabase accepts them in `apikey` and
  // can reject them when copied to Authorization. Legacy JWT keys still need it.
  if (!/^sb_(?:secret|publishable)_/.test(key) && !headers.Authorization)
